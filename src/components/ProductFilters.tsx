...
const FilterChip = ({ label, selected, onClick }: FilterChipProps) => (
  <motion.button
    ...
    className={`px-4 py-2 rounded-full border text-sm font-medium transition-all duration-200 ${
      selected
        ? "border-primary bg-primary/15 text-primary shadow-[0_0_12px_hsl(var(--primary)/0.15)]"
...
export const ProductFilters = ({
  ...
  const activeCount =
    filters.genders.length + filters.brands.length + filters.types.length;
  ...
  const clearAll = () =>
    onFiltersChange({ genders: [], brands: [], types: [] });
      ...
      <>
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <SheetTrigger asChild>
            ...
            >
              <Button
                className="rounded-full px-6 h-12 shadow-xl gap-2"
                size="lg"
              >
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl border border-border bg-card/50 backdrop-blur-sm p-6 space-y-5"
    >
      {filterContent}
